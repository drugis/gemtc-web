...
rancher kubectl delete secret gemtc-test-secrets -n drugis-test
rancher kubectl create secret generic gemtc-test-secrets \
  -n drugis-test \
...
rancher kubectl delete configmap gemtc-test-settings -n drugis-test
rancher kubectl create configmap gemtc-test-settings \
  -n drugis-test \
  --from-literal=GEMTC_AUTHENTICATION_METHOD=GOOGLE \
  --from-literal=GEMTC_DB_USERNAME=gemtctest \
  --from-literal=GEMTC_DB=gemtctest \
  --from-literal=GEMTC_HOST=https://gemtc-test.edge.molgenis.org \
