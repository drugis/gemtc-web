...
define([
  ...
  app.run([
    ...
    'HelpPopupService',
    'PageTitleService',
    function ($http, $q, $templateCache, HelpPopupService, PageTitleService) {
      $templateCache.put(
        'model-settings-section.html',
        require('../views/model-settings-section.html')
      );
      $templateCache.put(
        'convergence-diagnostics-section.html',
        require('../views/convergence-diagnostics-section.html')
      );
      $templateCache.put(
        'meta-regression-section.html',
        require('../views/meta-regression-section.html')
      );
      $templateCache.put(
        'results-section.html',
        require('../views/results-section.html')
      );
      $templateCache.put(
        'model-fit-section.html',
        require('../views/model-fit-section.html')
      );

      HelpPopupService.loadLexicon($http.get('lexicon.json'));
      PageTitleService.loadLexicon(
        $q.resolve(require('../gemtc-page-titles.json'))
      );
    }
  ]);
    ...
    function ($stateProvider, $urlRouterProvider, $httpProvider, $provide) {
      $provide.decorator('$uiViewScroll', function () {
        return function (uiViewElement) {
          window.scrollTo(
            0,
            uiViewElement[0].getBoundingClientRect().top - 200
          );
        };
      });
